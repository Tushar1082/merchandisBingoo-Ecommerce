...
export default function MyOrders() {
  ...
  function totalPrice(userItems){
    ...
    if(userItems){
      ...
      userItems.forEach((elem)=>{
      const discountPrice = parseFloat( (elem.price - (elem.price * (elem.sale / 100))).toFixed(2) );

        totprice+= (discountPrice * elem.quantity);
      })
      return totprice;
    }
  }
  ...
  function handleSearch(){
    ...
    console.log(data);
    data.forEach((orders)=>{
      orders.userOrders.map((obj)=>{
        const productName = (obj.name).toLowerCase().trim();
        str.forEach((elem)=>{
          const searchWord = elem.toLowerCase().trim();
